...
def create_quest_pair(
    ...
    Args:
        ...
        multiplayer: If True, quest state is synced across all players (Group="multiplayer")
        non_persistent: If True, quest resets when player leaves (Group="nonPersistent").
            Use for loops, heartbeats, and any quest that should restart each session.
            Ignored when multiplayer=True.
...
# ============================================================================
# Overrides (sync_room.py integration)
# ============================================================================

def load_overrides(overrides_path: str) -> Dict:
    """Load overrides.json if it exists. Returns empty dict if not found.

    Overrides are generated by tools/sync_room.py when a user makes manual
    edits in the Portals editor. Call this in generate.py and pass the result
    to apply_overrides() to preserve those edits across regeneration.

    Args:
        overrides_path: Path to overrides.json (e.g. "games/{room-id}/overrides.json")
...
def apply_overrides(room_data: Dict, overrides: Dict) -> Dict:
    """Apply overrides from sync_room.py to generated room data.

    Call this at the end of generate.py, BEFORE serialize_logic(), to preserve
    manual editor changes across regeneration. Mutates room_data in-place.

    Operation order: remove → modify → replace logic → add items
    This matches the merge_room.py convention and ensures consistency.

    Args:
        room_data: Generated room data with "roomItems" and "logic" dicts
        overrides: Loaded from overrides.json via load_overrides()

    Returns:
        Modified room_data (same reference, mutated in-place)
    """
    if not overrides:
        return room_data

    items = room_data.get("roomItems", {})
    logic = room_data.get("logic", {})
    quests = room_data.get("quests", {})
    settings = room_data.get("settings", {})

    # 1. Remove items (user deleted them in editor)
    for item_id in overrides.get("removed", []):
        item_id = str(item_id)
        items.pop(item_id, None)
        logic.pop(item_id, None)

    # 2. Modify items (field-level override — position, rotation, scale, etc.)
    for item_id, fields in overrides.get("modified", {}).items():
        item_id = str(item_id)
        if item_id in items:
    ...
    # 3. Replace logic (user changed effects/triggers in editor)
    for item_id, logic_entry in overrides.get("logic_replaced", {}).items():
        item_id = str(item_id)
        if logic_entry is None:
            logic.pop(item_id, None)
        else:
            logic[item_id] = logic_entry

    # 4. Add items (user created new items in editor)
    for item_id, entry in overrides.get("added", {}).items():
        item_id = str(item_id)
        items[item_id] = entry["item"]
        if entry.get("logic"):
            logic[item_id] = entry["logic"]

    # 5. Quest overrides
    quest_overrides = overrides.get("quests", {})
    if quest_overrides:
        for qid in quest_overrides.get("removed", []):
            quests.pop(str(qid), None)
        for qid, qdata in quest_overrides.get("modified", {}).items():
            quests[str(qid)] = qdata
        for qid, qdata in quest_overrides.get("added", {}).items():
            quests[str(qid)] = qdata

    # 6. Settings overrides
    for key, value in overrides.get("settings_modified", {}).items():
        settings[key] = value
